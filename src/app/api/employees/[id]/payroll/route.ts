...
export const GET = withHandler(async (req, ctx) => {
  const h = handlers.employeePayroll.GET;
  return h ? h(req, ctx) : methodNotAllowedResponse();
});
export const POST = withHandler(async (req, ctx) => {
  const h = handlers.employeePayroll.POST;
  return h ? h(req, ctx) : methodNotAllowedResponse();
});
export const PATCH = withHandler(async (req, ctx) => {
  const h = handlers.employeePayroll.PATCH;
  return h ? h(req, ctx) : methodNotAllowedResponse();
});
export const DELETE = withHandler(async (req, ctx) => {
  const h = handlers.employeePayroll.DELETE;
  return h ? h(req, ctx) : methodNotAllowedResponse();
});
